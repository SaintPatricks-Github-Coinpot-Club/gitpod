...
export class WorkspaceStarter {
    ...
    protected async newInstance(ctx: TraceContext, workspace: Workspace, user: User, excludeFeatureFlags: NamedWorkspaceFeatureFlag[], ideConfig: IDEConfig): Promise<WorkspaceInstance> {
        ...
        const client = WithReferrerContext.is(workspace.context) && ideConfig.clients?.[workspace.context.referrer];
        const clientDefaultOption = !!client && client.defaultDesktopIDE && ideConfig.ideOptions.options[client.defaultDesktopIDE]
        if (clientDefaultOption) {
            ...
            const option = userOption && client.desktopIDEs?.some(ide => ide === defaultDesktopIde) ? userOption : clientDefaultOption;
            configuration.desktopIdeImage = option.image;
            if (!defaultDesktopIde) {
                // A user does not have a default desktop IDE configured yet configure it with a client default.
                const additionalData = user?.additionalData || {};
                const settings = additionalData.ideSettings || {};
                settings.useDesktopIde = true;
                settings.defaultDesktopIde = client.defaultDesktopIDE;
